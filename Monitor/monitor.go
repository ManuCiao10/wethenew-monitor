...
func Webhook(client tls_client.HttpClient,new_id ID, idx int) {
		...
		Embeds: []Embeds{
			{
				...
				Fields: []Fields{
					...
					{
						Name:   "Brand",
						Value:  new_id.Results[idx].Brand,
						Inline: true,
					},
					// {
					// 	Name:   "Sizes",
					// 	Value: "diocane",
					// 	// Value:  "[" + new_id.Results[idx].SellNows[0].Size + " | " + strconv.Itoa(new_id.Results[idx].SellNows[0].Price) + " €" + "]" + "(" + fmt.Sprintf("https://sell.wethenew.com/sell-now/%d?holding-Lab", new_id.Results[idx].SellNows[0].ID) + ")",
					// 	Inline: true,
					// },
				},
				Thumbnail: Thumbnail{
	...
	fmt.Println(sendWebhookRes.StatusCode)
